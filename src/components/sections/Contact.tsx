...
export const Contact = () => {
  ...
  const rawLocations = contact.locations && contact.locations.length > 0
    ? contact.locations
    : [{ studioName: contact.studioName, address: contact.address, mapsUrl: contact.mapsUrl, hours: contact.hours }];
  const locations = rawLocations.filter((l) => l && (l.studioName?.trim() || l.address?.trim() || (l.hours?.length ?? 0) > 0));
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="space-y-5">
          {locations.map((loc, idx) => {
            const mapsUrl = loc.mapsUrl?.trim() || "https://maps.google.com/?q=" + encodeURIComponent(loc.address || "");
            return (
              <div key={idx} className="glass rounded-2xl p-6 md:p-8 maroon-glow">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div className="flex-1">
                    <p className="text-xs tracking-[0.25em] uppercase text-accent mb-1">Studio {locations.length > 1 ? idx + 1 : ""}</p>
                    <h3 className="font-serif text-xl mb-2">{loc.studioName}</h3>
